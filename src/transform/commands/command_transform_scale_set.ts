...
/** Snapshot of an object's scale for CommandTransformSetScale undo/restore. */
...
/**
 * Undoable command that sets the scale of one or more objects. Snapshots
 * original scales on construction and restores them on undo.
 */
export class CommandTransformScaleSet implements UndoCommand {
  private snapshots: SetScaleSnapshot[];
  private newScales: THREE.Vector3[];

  /**
   * Creates a new set scale command. Captures each object's current scale as
   * the undo state.
   *

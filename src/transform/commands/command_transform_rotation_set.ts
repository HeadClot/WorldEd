...
/**
 * Snapshot of an object's rotation for CommandTransformSetRotation
 * undo/restore.
 */
...
/**
 * Undoable command that sets the rotation of one or more objects. Snapshots
 * original rotations on construction and restores them on undo.
 */
export class CommandTransformRotationSet implements UndoCommand {
  private snapshots: SetRotationSnapshot[];
  private newRotations: THREE.Euler[];

  /**
   * Creates a new set rotation command. Captures each object's current rotation
   * as the undo state.
   *
   * @param objects The objects whose rotations will be changed.
   * @param newRotations The target Euler rotations corresponding to each
   *   object.
   */

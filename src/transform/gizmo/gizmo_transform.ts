...
import { GizmoTranslate } from './gizmo_translate.js';
import { GizmoRotate } from './gizmo_rotate.js';
import { GizmoScale } from './gizmo_scale.js';
...
import type { BoundsFaceHighlightMode } from '@/transform/bounds/bounds_face_highlight.js';
import {
...
export class GizmoTransform {
  ...
  private gizmoTranslate: GizmoTranslate;
  private gizmoRotate: GizmoRotate;
  private gizmoScale: GizmoScale;
  private gizmoBounds: GizmoBounds;
  ...
  constructor(theme: typeof Theme) {
    ...
    this.handleGroup.name = 'gizmo_transform';
    this.viewportGroups = [];
    this.viewportPlanes = [];
    this.currentHandles = [];
    this.activeHandle = null;
    this.gizmoTranslate = new GizmoTranslate(theme);
    this.gizmoRotate = new GizmoRotate(theme);
    this.gizmoScale = new GizmoScale(theme);
    this.gizmoBounds = new GizmoBounds(theme);
  ...
  updateBoundsFromMeshes(meshes: THREE.Mesh[], camera: THREE.Camera | null = null): void {
    ...
    this.resetHandleGroupTransform();
    this.gizmoBounds.updateFromBounds(bounds, cubePickSize, earSize, cubeVisualSize);
  ...
  prepareBoundsCloneForCamera(
    ...
    this.gizmoBounds.applyScreenSpaceStyleToClone(group, viewPlane, camera, viewportHeightPx);
  ...
  getCurrentBounds(): DataOrientedBounds | null {
    return this.gizmoBounds.getCurrentBounds();
  }
  ...
  setBoundsGuideLinesVisible(visible: boolean): void {
    this.gizmoBounds.setGuideLinesVisible(visible);
    this.lastBoundsPoseSignature = '';
    this.syncMasterTransformToClones();
  }
  ...
  setBoundsResizeHandlesVisible(visible: boolean): void {
    this.gizmoBounds.setResizeHandlesVisible(visible);
    this.lastBoundsPoseSignature = '';
    this.syncMasterTransformToClones();
  }
  ...
  setHighlightedBoundsFace(face: BoundsFace | null, mode: BoundsFaceHighlightMode = 'resize'): void {
    if (this.currentMode !== TransformMode.BOUNDS) return;
    if (this.gizmoBounds.getHighlightedFace() === face && this.gizmoBounds.getHighlightMode() === mode) {
      return;
    }
    this.gizmoBounds.setHighlightedFace(face, mode);
  ...
  getHighlightedBoundsFace(): BoundsFace | null {
    return this.gizmoBounds.getHighlightedFace();
  }
  ...
  dispose(): void {
    this.gizmoTranslate.dispose();
    this.gizmoRotate.dispose();
    this.gizmoScale.dispose();
    this.gizmoBounds.dispose();
  ...
  private populateMasterGroup(mode: TransformMode): void {
    if (mode === TransformMode.TRANSLATE) {
      this.attachModeGizmo(this.gizmoTranslate);
      return;
    }
    if (mode === TransformMode.ROTATE) {
      this.attachModeGizmo(this.gizmoRotate);
      return;
    }
    if (mode === TransformMode.SCALE) {
      this.attachModeGizmo(this.gizmoScale);
      return;
    }
    if (mode === TransformMode.BOUNDS) {
      this.attachModeGizmo(this.gizmoBounds);
    }
  }
  ...
  private cloneHandleGroupContents(): THREE.Group {
    ...
    clone.name = 'gizmo_transform_viewport';
  ...
  private syncMasterTransformToClones(): void {
    this.viewportGroups.forEach((group, index) => {
      ...
      this.gizmoBounds.applyHighlightToRoot(group, viewPlane === 'xyz');
    });
  }
   ...
   */
  private buildBoundsPoseSignature(bounds: DataOrientedBounds | null): string {
    const guides = this.gizmoBounds.areGuideLinesVisible() ? '1' : '0';
    const grips = this.gizmoBounds.areResizeHandlesVisible() ? '1' : '0';
  ...
  private applyViewPlaneFilters(group: THREE.Group, viewPlane: CadViewPlane): void {
    if (this.currentMode === TransformMode.BOUNDS) {
      this.gizmoBounds.styleCloneForViewPlane(group, viewPlane);
      return;
    }

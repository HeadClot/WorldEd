 ...
 */
export function getEditorStartupErrorMessage(error: unknown): string {
...
function createStartupErrorOverlay(error: unknown): HTMLElement {
  ...
  overlay.appendChild(createStartupErrorDetails(error));
  return overlay;
}
 ...
 */
function createStartupErrorDetails(error: unknown): HTMLElement {
  const details = document.createElement('code');
  details.textContent = getEditorStartupErrorMessage(error);

...
import { ScaleWidget } from '../widgets/scale_widget.js';
...
/**
 * Scale tool matching Shape Editor ScaleTool: permanent widget mode and
 * single-use scale via UseTool / S.
 */
export class ScaleTool extends BoxSelectTool {
  ...
  /**
   * Routes Escape cancel and modal axis/numeric keys during single-use scale.
   *
   * @param keyCode Key code string.
   * @param event Optional original browser keyboard event.
   * @returns True when consumed.
  ...
  /** Permanent path: show scale gizmo and attach scale widget. */
  private activateWidgetPath(): void {
    const services = this.editor?.getServices();
    if (!services || !this.editor) {
      return;
    }
    services.setWidgetMode(TransformMode.SCALE);
    services.refreshGizmoPresentation();
    services.setStatusMessage('Scale');
    this.editor.addWidget(this.scaleWidget);
    this.scaleWidget.onBeginScaling = () => this.toolOnBeginScaling();
    this.updateWidgetVisibility();
  }

  /** Updates scale widget visibility from selection. */
  private updateWidgetVisibility(): void {
    if (!this.editor) {
      return;
    }
    if (this.editor.selectedSegmentsCount > 0) {
      this.scaleWidget.position.copy(this.editor.selectedSegmentsAveragePosition);
      this.scaleWidget.visible = true;
      return;
    }
    this.scaleWidget.visible = false;
  }

  /** Begins scaling: register undo and start single-use drag when applicable. */
  private toolOnBeginScaling(): void {
    if (!this.editor) {
      return;
    }
    this.editor.registerUndo('Scale Selection');
    if (!this.isSingleUse) {
      return;
    }
  ...
  /** Applies live mouse-driven scale during single-use. */
  ...
  private beginSingleUseDragFromServices(): void {
    ...
    services.setStatusMessage('Scale (single-use)');
  }
}

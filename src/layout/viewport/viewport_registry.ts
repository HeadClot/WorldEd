import { createViewportForKind, type ViewportFactoryDependencies } from '@/viewports/core/factory_viewport.js';
...
} from '@/viewports/core/viewport_kind.js';
...
export class ViewportRegistry {
   ...
   */
  private activeRenderViewports: ViewportEditor[];
   ...
   */
  getAllViewports(): ViewportEditor[] {
    return this.panes
      .map((pane) => pane.getViewport())
      .filter((viewport): viewport is ViewportEditor => viewport !== null);
  }
   ...
   */
  replaceKind(paneId: string, kind: ViewportKind): ViewportEditor | null {
  ...
  private rebuildActiveRenderViewports(): void {
    const orthographic: ViewportEditor[] = [];
    const perspective: ViewportEditor[] = [];
   ...
   */
  private createViewportInPane(pane: ViewportPane, kind: ViewportKind): ViewportEditor | null {

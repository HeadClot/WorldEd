...
import { areEditorStorageWritesSuppressed } from '@/settings/storage/clear_editor_storage.js';
...
export interface WorkspaceAreaWiringHost {
  ...
  getWorkspaceController(): ControllerWorkspace | null;
...
export function wireWorkspaceSystem(host: WorkspaceAreaWiringHost): void {
  const controller = new ControllerWorkspace(
...
export function handleAreaLayoutStructureChanged(host: WorkspaceAreaWiringHost): void {
  ...
  host
    ...
    .forEach((pane: ViewportPane) => {
      const viewport = pane.getViewport();
      if (!viewport) return;
      wireToolbarForPane(host.getViewportChromeHost(), pane, viewport as ViewportEditor);
    });

 ...
 * Produces CommandTransformAlign instances for full undo/redo support.
 */
export class ControllerAlignment {
  ...
  cycleAxisRestriction(): AlignmentAxis {
    const currentIndex = ControllerAlignment.AXIS_CYCLE.indexOf(this.currentAxis);
    const nextIndex = (currentIndex + 1) % ControllerAlignment.AXIS_CYCLE.length;
    this.currentAxis = ControllerAlignment.AXIS_CYCLE[nextIndex]!;
    return this.currentAxis;
  }
  ...
  alignToOrigin(objects: THREE.Mesh[], axis: AlignmentAxis, commandStack: CommandStack): void {
    ...
    const command = new CommandTransformAlign(snapshots, targetPositions);

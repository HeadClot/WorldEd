...
import { ReparentCommand } from '../../commands/object/reparent_command.js';
import { ReparentMove, ReparentObjectsCommand } from '../../commands/object/reparent_objects_command.js';
import { isDescendantOf, sortObjectsBySceneOrder } from '../../utils/hierarchy_utils.js';
...
export class HierarchyReparentHandler {
  ...
  private commitMoves(moves: readonly ReparentMove[]): void {
    if (moves.length === 1) {
      const only = moves[0]!;
      this.commandStack.push(new ReparentCommand(only.object, only.newParent, only.insertBefore));
    } else {
      this.commandStack.push(new ReparentObjectsCommand(moves));
    }

...
/**
 * Expands a face pick to one selectable face unit.
 * Solid CSG results stay within one authored brush face (brushId + surfaceIndex)
 * so adjacent walls and carpet/detail brushes remain independently selectable.
 * Ordinary meshes expand to edge-connected coplanar triangles.
...
/**
 * Returns all result triangles that share the seed's brush id and surface index.
 * When the mesh has solid sources but the seed row is invalid, returns only the seed
 * (never coplanar flood-fill across unrelated brushes).
 * @param mesh Candidate solid result mesh.
 * @param seedFaceIndex Seed triangle.
 * @returns Sorted triangle indices, or null when the mesh is not a solid result.
 */
 ...
 * @param sources Per-triangle solid source rows (same order as result triangles).

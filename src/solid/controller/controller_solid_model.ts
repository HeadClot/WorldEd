...
import { CommandSolidReorderBrushes, SolidBrushOrderEnd } from '@/solid/commands/command_solid_reorder_brushes.js';
...
export class ControllerSolidModel {
  ...
  private placeModelInScene(model: SolidModel, selectTarget: THREE.Object3D, statusMessage: string): void {
    const command = new CommandCreateSolidModel(model, this.worldObject);
  ...
  setBrushOperationForMeshes(meshes: THREE.Mesh[], operation: SolidOperation): void {
    ...
    const command = new CommandSolidSetBrushOperation(meshes, operation);
    this.commandStack.push(command);
    this.panel.refresh();
    this.syncViewports?.();
    this.refreshOutliner?.();
  ...
  setGroupOperationForGroups(groups: THREE.Group[], operation: SolidOperation): void {
    ...
    const command = new CommandSolidSetGroupOperation(groups, operation);
    this.commandStack.push(command);
    this.panel.refresh();
    this.syncViewports?.();
    this.refreshOutliner?.();
  ...
  moveBrushesInOrder(nodes: THREE.Object3D[], end: SolidBrushOrderEnd): void {
    ...
    const command = new CommandSolidReorderBrushes(reorderNodes, end);
    this.commandStack.push(command);
    this.panel.refresh();
    this.syncViewports?.();
    this.refreshOutliner?.();

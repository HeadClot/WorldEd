...
import type { SolidCsgTree, SolidCsgTreeNode } from './solid_csg_tree.js';
import { SOLID_ROUTING_CATEGORY_COUNT, SolidRoutingTable, type SolidRoutingStep } from './solid_routing_table.js';
 ...
 */
export class SolidRoutingTableBuilder {
  /**
  ...
  static buildLinearTable(
    ...
    return new SolidRoutingTable(steps, invertedWorld);
  }
  ...
  private static buildOptimizedStep(
    ...
    const categoryRows: number[][] = inputCategories.map((left) => {
      const row: number[] = [];
      for (let col = 0; col < SOLID_ROUTING_CATEGORY_COUNT; col++) {
  ...
  private static renumberOutputsToNextStates(categoryRows: number[][]): {
    ...
    const indexedRows = categoryRows.map((row) => {
      const out = new Uint8Array(SOLID_ROUTING_CATEGORY_COUNT);
      for (let col = 0; col < SOLID_ROUTING_CATEGORY_COUNT; col++) {

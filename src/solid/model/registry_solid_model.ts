 ...
 */
export class RegistrySolidModel {
  ...
  static register(root: THREE.Object3D, model: SolidModel): void {
    RegistrySolidModel.registry.set(root, model);
  }
  ...
  static get(object: THREE.Object3D): SolidModel | undefined {
    return RegistrySolidModel.registry.get(object);
  }
  ...
  static fromObject(object: THREE.Object3D): SolidModel | null {
    const direct = RegistrySolidModel.registry.get(object);
    if (direct) return direct;
    let current: THREE.Object3D | null = object.parent;
    while (current) {
      const model = RegistrySolidModel.registry.get(current);
  ...
  static collectUnder(root: THREE.Object3D): Set<SolidModel> {
    ...
    root.traverse((object) => {
      ...
      const model = RegistrySolidModel.registry.get(object);

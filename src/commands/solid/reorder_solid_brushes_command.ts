...
export class ReorderSolidBrushesCommand implements UndoCommand {
  ...
  execute(): void {
    ...
    byModel.forEach((meshes, model) => {
      ...
      const brushIds = meshes.map((mesh) => model.findBrushByMesh(mesh)?.id).filter((id): id is string => !!id);
      if (brushIds.length === 0) return;
      const changed = this.end === 'first' ? model.moveBrushesToFirst(brushIds) : model.moveBrushesToLast(brushIds);
      if (!changed) return;

 ...
 */
export function captureViewportCameraSnapshot(viewport: ViewportEditor): ViewportCameraSnapshot | null {
 ...
 */
export function applyViewportCameraSnapshot(viewport: ViewportEditor, snapshot: ViewportCameraSnapshot): boolean {
  const camera = viewport.getCamera();
 ...
 * @param viewport Target viewport.
 */
function syncFlyingOrientationIfPresent(viewport: ViewportEditor): void {
  const candidate = viewport as ViewportEditor & { syncFlyingCameraOrientation?: () => void };

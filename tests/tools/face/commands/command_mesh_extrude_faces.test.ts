...
import { CommandMeshExtrudeFaces } from '@/tools/face/commands/command_mesh_extrude_faces.js';
import { CommandStack } from '@/commands/command_stack.js';

describe('CommandMeshExtrudeFaces', () => {
  ...
  it('should add all created meshes to the parent on execute', () => {
    const command = CommandMeshExtrudeFaces.fromMeshes([meshA, meshB], parent);
    command.execute();
  ...
  it('should work with the command stack', () => {
    ...
    stack.push(CommandMeshExtrudeFaces.fromMeshes([meshA, meshB], parent));
    expect(parent.children.length).toBe(2);
    stack.undo();

...
import { CommandMeshFacesExtrude } from '@/tools/face/commands/command_mesh_faces_extrude.js';
import { CommandStack } from '@/commands/command_stack.js';

describe('CommandMeshFacesExtrude', () => {
  ...
  it('should add all created meshes to the parent on execute', () => {
    const command = CommandMeshFacesExtrude.fromMeshes([meshA, meshB], parent);
    command.execute();
  ...
  it('should work with the command stack', () => {
    ...
    stack.push(CommandMeshFacesExtrude.fromMeshes([meshA, meshB], parent));
    expect(parent.children.length).toBe(2);
    stack.undo();

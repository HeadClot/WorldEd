...
function buildNestedBoxVmf(): string {
  const outer = buildAxisAlignedSideBlocks({ x: -128, y: -128, z: -128 }, { x: 128, y: 128, z: 128 }, 'DEV/OUTER');
  const inner = buildAxisAlignedSideBlocks({ x: -32, y: -32, z: -32 }, { x: 32, y: 32, z: 32 }, 'DEV/INNER');
  return `
world
{
	"id" "1"

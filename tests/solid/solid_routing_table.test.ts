...
import { SolidFragmentRouter } from '../../src/solid/algorithm/solid_fragment_router.js';
...
describe('SolidRoutingTable', () => {
  it('matches sequential routing for additive then subtractive', () => {
    ...
    const table = SolidRoutingTableBuilder.buildForSubject(prepared, 0, [1], tree, false, false);
  ...
  it('keeps subject self-aligned surface when peer is outside', () => {
    ...
    const tree = SolidCsgTree.fromPreparedFlat(prepared);
    const table = SolidRoutingTableBuilder.buildForSubject(prepared, 0, [], tree, false, false);

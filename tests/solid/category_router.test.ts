...
describe('CategoryRouter', () => {
  it('routes additive outside+outside to outside', () => {
    const result = CategoryRouter.route(SurfaceCategory.Outside, SurfaceCategory.Outside, SolidOperation.Additive);
    expect(result).toBe(SurfaceCategory.Outside);
  });

  it('routes additive outside+selfAligned to selfAligned', () => {
    const result = CategoryRouter.route(SurfaceCategory.Outside, SurfaceCategory.SelfAligned, SolidOperation.Additive);
  ...
  it('cancels subject selfAligned against peer aligned for additive union', () => {
    ...
    const afterPeer = CategoryRouter.route(afterSelf, SurfaceCategory.Aligned, SolidOperation.Additive);
  ...
  it('keeps later brush when peer aligned then selfAligned for additive', () => {
    const afterPeer = CategoryRouter.route(SurfaceCategory.Outside, SurfaceCategory.Aligned, SolidOperation.Additive);
    const afterSelf = CategoryRouter.route(afterPeer, SurfaceCategory.SelfAligned, SolidOperation.Additive);
  ...
  it('routes subtractive inside+inside to outside', () => {
    const result = CategoryRouter.route(SurfaceCategory.Inside, SurfaceCategory.Inside, SolidOperation.Subtractive);
    expect(result).toBe(SurfaceCategory.Outside);
  });

  it('routes intersecting inside+outside to outside', () => {
    const result = CategoryRouter.route(SurfaceCategory.Inside, SurfaceCategory.Outside, SolidOperation.Intersecting);
    expect(result).toBe(SurfaceCategory.Outside);
  });
});

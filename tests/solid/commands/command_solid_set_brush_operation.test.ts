...
describe('CommandSolidSetBrushOperation', () => {
  it('changes operation and restores the prior value on undo', () => {
    const model = new SolidModel('OpSolid');
    const brush = model.addBoxBrush(2, SolidOperation.Additive);
    const command = new CommandSolidSetBrushOperation([brush.mesh!], SolidOperation.Subtractive);
    command.execute();
  ...
  it('is a no-op when the operation is already active', () => {
    ...
    const brush = model.addBoxBrush(2, SolidOperation.Additive);
    const command = new CommandSolidSetBrushOperation([brush.mesh!], SolidOperation.Additive);
    command.execute();
    command.undo();
    expect(model.findBrush(brush.id)?.operation).toBe(SolidOperation.Additive);
  });

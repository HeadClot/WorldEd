...
import {
  CommandSolidReorderBrushes,
  reorderSolidContentSiblings,
} from '@/solid/commands/command_solid_reorder_brushes.js';
...
describe('CommandSolidReorderBrushes', () => {
  it('moves a brush to first and restores order on undo', () => {
    ...
    stack.push(new CommandSolidReorderBrushes([c.mesh!], 'first'));
  ...
  it('moves a brush to last and preserves multi-select relative order', () => {
    ...
    const stack = new CommandStack(8);
    stack.push(new CommandSolidReorderBrushes([a.mesh!, b.mesh!], 'last'));
  ...
  it('moves a nested brush to last inside its group without moving the group', () => {
    ...
    const stack = new CommandStack(8);
    stack.push(new CommandSolidReorderBrushes([nestedFirst.mesh!], 'last'));
  ...
  it('moves a solid CSG group to last among root content siblings', () => {
    ...
    model.syncBrushOrderFromScene();
    const stack = new CommandStack(8);
    stack.push(new CommandSolidReorderBrushes([group], 'last'));

...
} from '../../../src/managers/layout/scene_visual_refresh.js';
...
describe('properties panel transform commit hook', () => {
  it('invokes afterTransformCommit when position changes', async () => {
    ...
    const { SelectionManager } = await import('../../../src/selection/object/selection_manager.js');
    const { PropertiesPanel } = await import('../../../src/ui/properties/properties_panel.js');
    const { CommandStack } = await import('../../../src/commands/command_stack.js');
    const container = document.createElement('div');
    document.body.appendChild(container);
    const selectionManager = new SelectionManager();
    const panel = new PropertiesPanel(container, Theme, selectionManager);

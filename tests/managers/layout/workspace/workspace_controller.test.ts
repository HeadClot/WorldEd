...
import { createDualTopPerspectiveLayout } from '../../../../src/managers/layout/area/area_layout_presets.js';
import { listAreaLeafPlacements } from '../../../../src/managers/layout/area/area_layout_tree.js';
import { WorkspaceController } from '../../../../src/managers/layout/workspace/workspace_controller.js';
 ...
 * @returns Viewport stub.
 */
function createMockViewport(kind: ViewportKind, camera?: THREE.Camera): EditorViewport {
 ...
 */
function createDualAreaController(): { controller: AreaLayoutController; layer: HTMLElement } {
  ...
  const controller = new AreaLayoutController(layer, createDualTopPerspectiveLayout());
...
describe('WorkspaceController layout persistence', () => {
  it('persists viewport kind into the active workspace when the tree is updated', () => {
    ...
    const workspace = new WorkspaceController(store, areaController, registry, host);
    ...
    const area2 = new AreaLayoutController(layer, createDualTopPerspectiveLayout());
    ...
    const workspace2 = new WorkspaceController(reloaded, area2, registry2, host2);

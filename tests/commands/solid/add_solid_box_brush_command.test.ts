...
import { AddSolidBoxBrushCommand } from '../../../src/commands/solid/add_solid_box_brush_command.js';
...
describe('AddSolidBoxBrushCommand', () => {
  ...
  it('appends a new brush under a solid CSG group when parent is supplied', () => {
    ...
    model.rebuild(true);
    const command = new AddSolidBoxBrushCommand(model, 2, SolidOperation.Additive, new THREE.Vector3(0, 0, 0), group);
    command.execute();
    const created = command.getCreatedBrush();

...
} from '../../../src/commands/solid/reorder_solid_brushes_command.js';
...
describe('ReorderSolidBrushesCommand', () => {
  ...
  it('moves a nested brush to last inside its group without moving the group', () => {
    ...
    const stack = new CommandStack(8);
    stack.push(new ReorderSolidBrushesCommand([nestedFirst.mesh!], 'last'));
  ...
  it('moves a solid CSG group to last among root content siblings', () => {
    ...
    model.syncBrushOrderFromScene();
    const stack = new CommandStack(8);
    stack.push(new ReorderSolidBrushesCommand([group], 'last'));

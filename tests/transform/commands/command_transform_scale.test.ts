...
describe('CommandTransformScale', () => {
  ...
  it('should execute and apply scale relative to pivot', () => {
    const command = new CommandTransformScale(snapshots, pivot, axis, factor);
    command.execute();
  ...
  it('should handle per-axis scaling along Y axis', () => {
    ...
    const command = new CommandTransformScale(ySnapshots, pivot, yAxis, 3, GizmoAxis.Y);
    command.execute();
  ...
  it('should handle scale factor of 1 as no-op', () => {
    ...
    const command = new CommandTransformScale(snapshots, pivot, axis, identityFactor);
    command.execute();
    expect(mesh1.position.x).toBeCloseTo(2);
    expect(mesh1.position.y).toBeCloseTo(0);
    expect(mesh1.scale.x).toBeCloseTo(1);

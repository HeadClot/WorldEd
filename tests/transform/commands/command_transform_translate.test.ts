...
describe('CommandTransformTranslate', () => {
  ...
  it('should execute and move objects by delta', () => {
    const command = new CommandTransformTranslate(snapshots, delta);
    command.execute();
  ...
  it('should be a no-op when delta is zero', () => {
    ...
    const command = new CommandTransformTranslate(snapshots, zeroDelta);
    command.execute();
    expect(mesh1.position.x).toBeCloseTo(1);
    expect(mesh1.position.y).toBeCloseTo(2);
    expect(mesh1.position.z).toBeCloseTo(3);
  ...
  it('should prefer finalPosition over delta when provided', () => {
    ...
    const command = new CommandTransformTranslate(finalSnapshots, new THREE.Vector3(100, 0, 0));
    command.execute();

...
describe('CommandTransformRotate', () => {
  ...
  it('should execute and apply rotation around pivot', () => {
    const command = new CommandTransformRotate(snapshots, pivot, axis, angle);
    command.execute();
  ...
  it('should work with a non-origin pivot', () => {
    ...
    const command = new CommandTransformRotate(snapshots2, offPivot, axis, angle);
    command.execute();
    command.undo();
    expect(mesh1.position.x).toBeCloseTo(2);
    expect(mesh1.position.y).toBeCloseTo(0);

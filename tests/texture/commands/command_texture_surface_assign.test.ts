...
describe('CommandTextureSurfaceAssign', () => {
  beforeEach(() => {
  ...
  it('should assign texture id on execute and restore on undo', () => {
    ...
    const command = new CommandTextureSurfaceAssign(targets, 'walls/brick.png');
    command.execute();
  ...
  it('should preserve UV scale when only the texture id changes', () => {
    ...
    const targets = buildTargetsFromMeshes([mesh]);
    const command = new CommandTextureSurfaceAssign(targets, 'floor.png');
    command.execute();

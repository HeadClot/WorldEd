...
import { CommandTextureUvStrokeSmear } from '@/texture/commands/command_texture_uv_stroke_smear.js';
...
describe('CommandTextureUvStrokeSmear', () => {
  it('undo restores solid brush face mappings after a live smear stroke', () => {
    ...
    const before = CommandTextureUvStrokeSmear.captureMesh(result);
    ...
    const after = CommandTextureUvStrokeSmear.captureMesh(result);
    expect(after.solidBrushUvs![0]!.faceMappings[0]?.textureId).toBe('smeared.png');

    const stack = new CommandStack(16);
    stack.recordExecuted(new CommandTextureUvStrokeSmear([before], [after]));
  ...
  it('captures solid brush UVs in mesh snapshots', () => {
    ...
    model.rebuild(true);
    const snapshot = CommandTextureUvStrokeSmear.captureMesh(model.getResultMesh());
  ...
  it('undo restores UV maps on a plain mesh without solid brushes', () => {
    ...
    const before = CommandTextureUvStrokeSmear.captureMesh(mesh);
    const uv = mesh.geometry.getAttribute('uv') as THREE.BufferAttribute;
    (uv.array as Float32Array).fill(0.9);
    uv.needsUpdate = true;
    const after = CommandTextureUvStrokeSmear.captureMesh(mesh);
    const stack = new CommandStack(8);
    stack.push(new CommandTextureUvStrokeSmear([before], [after]));
    stack.undo();

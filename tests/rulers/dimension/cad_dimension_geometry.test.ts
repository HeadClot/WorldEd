...
import { createFixedCadPlacementContext } from '@/rulers/dimension/cad_placement_context.js';
 ...
 */
function makeBounds(halfX: number, halfY: number, halfZ: number): DataOrientedBounds {
  return {
 ...
 * @returns Oriented bounds data.
 */
function makeBoundsAt(center: THREE.Vector3, halfX: number, halfY: number, halfZ: number): DataOrientedBounds {
  return {
    center: center.clone(),
    quaternion: new THREE.Quaternion(),
    halfExtents: new THREE.Vector3(halfX, halfY, halfZ),

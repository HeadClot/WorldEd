...
describe('CommandObjectDelete', () => {
  ...
  it('should execute and remove meshes from parent', () => {
    const command = new CommandObjectDelete(snapshots);
    command.execute();
  ...
  it('should undo and restore full state for single mesh', () => {
    ...
    const command = new CommandObjectDelete([singleSnapshot]);
    command.execute();
  ...
  it('should handle mesh with no parent gracefully', () => {
    ...
    const command = new CommandObjectDelete([orphanSnapshot]);
    command.execute();
    command.undo();

...
describe('CommandObjectGroup', () => {
  ...
  it('should execute and create a group containing the children', () => {
    const command = new CommandObjectGroup([mesh1, mesh2], parent, 'TestGroup');
    command.execute();
  ...
  it('should work with command stack for full undo/redo cycle', () => {
    ...
    const command = new CommandObjectGroup([mesh1, mesh2], parent, 'StackGroup');
    stack.push(command);
    expect(parent.children.length).toBe(1);
    stack.undo();
    expect(parent.children.length).toBe(2);
  ...
  it('should group single object', () => {
    const command = new CommandObjectGroup([mesh1], parent, 'SingleGroup');
    command.execute();
    const group = command.getGroup();
  ...
  it('should handle objects from different parents', () => {
    ...
    const command = new CommandObjectGroup([mesh1, mesh3], parent, 'MixedGroup');
    command.execute();
    const group = command.getGroup();
    expect(group.children.length).toBe(2);
    expect(group.children).toContain(mesh1);
  ...
  it('should restore sibling order on undo', () => {
    ...
    mesh3.name = 'Mesh3';
    parent.add(mesh3);
    const command = new CommandObjectGroup([mesh1, mesh2], parent, 'OrderGroup');
    command.execute();
    command.undo();
  ...
  it('should preserve object transforms on group/ungroup', () => {
    ...
    const command = new CommandObjectGroup([mesh1], parent, 'TransformGroup');
    command.execute();
    command.undo();
  ...
  it('should return the group via getGroup method', () => {
    const command = new CommandObjectGroup([mesh1], parent, 'GetGroup');
    const group = command.getGroup();
  ...
  it('should not execute twice', () => {
    const command = new CommandObjectGroup([mesh1, mesh2], parent, 'OnceGroup');
    command.execute();
    command.execute();
    expect(parent.children.length).toBe(1);
  });
  ...
  it('should no-op undo when execute has not run', () => {
    const command = new CommandObjectGroup([mesh1, mesh2], parent, 'GuardGroup');
  ...
  it('should nest an existing group under a new group without emptying it', () => {
    ...
    inner.add(mesh1);
    parent.remove(mesh1);
    const command = new CommandObjectGroup([inner], parent, 'Outer');
    command.execute();
  ...
  it('should insert the group at the earliest selected sibling index, not at the end', () => {
    ...
    mesh3.name = 'Mesh3';
    parent.add(mesh3);
    const command = new CommandObjectGroup([mesh1], parent, 'AtClickSite');
    command.execute();
    const group = command.getGroup();
    expect(parent.children[0]).toBe(group);
    expect(parent.children[1]).toBe(mesh2);
  ...
  it('should insert a multi-member group at the topmost member index', () => {
    ...
    parent.add(mesh3);
    parent.add(mesh4);
    const command = new CommandObjectGroup([mesh2, mesh3], parent, 'MidGroup');
    command.execute();
    const group = command.getGroup();
    expect(parent.children[0]).toBe(mesh1);
  ...
  it('should keep middle insert position across undo and redo', () => {
    ...
    mesh3.name = 'Mesh3';
    parent.add(mesh3);
    const command = new CommandObjectGroup([mesh2], parent, 'RedoSite');
    command.execute();

...
describe('CommandObjectColorSet', () => {
  it('should apply the new color on execute', () => {
    const mesh = createColoredMesh(0xff0000);
    const command = new CommandObjectColorSet([mesh], 0x00ff00);
    command.execute();
  ...
  it('should restore the original color on undo', () => {
    ...
    const command = new CommandObjectColorSet([mesh], 0xaabbcc);
    command.execute();
    command.undo();
  ...
  it('should re-apply the new color on redo execute', () => {
    ...
    const command = new CommandObjectColorSet([mesh], 0xffffff);
    command.execute();
    command.undo();
    command.execute();
  ...
  it('should update every mesh in a multi-selection', () => {
    ...
    const command = new CommandObjectColorSet([meshA, meshB], 0xabcdef);
    command.execute();
  ...
  it('should snapshot colors at construction time', () => {
    ...
    const command = new CommandObjectColorSet([mesh], 0xfefefe);
  ...
  it('should report affected mesh count for colorable meshes only', () => {
    ...
    const command = new CommandObjectColorSet([mesh], 0x00ffff);
  ...
  it('should use explicit original colors when provided after live preview', () => {
    ...
    const command = new CommandObjectColorSet([mesh], 0x333333, [0x111111]);
    command.execute();
  ...
  it('should update the target color for live picker coalescing', () => {
    ...
    const command = new CommandObjectColorSet([mesh], 0x020202);
    command.execute();

...
describe('CommandObjectReparent', () => {
  ...
  it('should reparent a mesh under a group on execute', () => {
    const command = new CommandObjectReparent(meshA, groupA);
    command.execute();
  ...
  it('should insert before a sibling when requested', () => {
    const command = new CommandObjectReparent(meshB, world, meshA);
    command.execute();
  ...
  it('should reject reparenting an object into itself', () => {
    const command = new CommandObjectReparent(meshA, meshA);
    command.execute();
    expect(meshA.parent).toBe(world);
  });
});
